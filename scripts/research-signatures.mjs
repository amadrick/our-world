...
// (signatureSubject, signatureRationale), plus a drawable description in
// data/place-image-hints.json for `npm run images`.
//
...
//   npm run signatures -- --images         also draw each researched place's still
...
for (const place of targets) {
  ...
  if (found.visual) hints[place.id] = { visual: found.visual, scene: found.scene };
  await save();

  if (args.images && !args.dryRun) {
    try {
      const hint = { ...fallbackHint(place), ...hints[place.id] };
      place.image = await saveImage(await renderImage(buildPrompt(hint)), place.id, ROOT);
      await save();
