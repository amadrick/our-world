...
//   npm run images -- --colors                     resample each page color from its saved image
//
...
import { renderImage, sampleImageColor, saveImage } from "../src/lib/images/render.mjs";
...
function parseArgs(argv) {
  const args = { ids: [], all: false, print: false, colors: false, out: null, photo: null, import: null, importDir: null };
  for (let i = 0; i < argv.length; i++) {
    const arg = argv[i];
    if (arg === "--all") args.all = true;
    else if (arg === "--print") args.print = true;
    else if (arg === "--colors") args.colors = true;
...
if (args.colors) {
  for (const place of data.places) {
    if (place.image) place.imageColor = await sampleImageColor(path.join(ROOT, "public", place.image));
  }
  await writeFile(PLACES_FILE, `${JSON.stringify(data, null, 2)}\n`);
  console.log(`Sampled ${data.places.filter((p) => p.imageColor).length} page colors.`);
  process.exit(0);
}
...
for (const place of targets) {
  try {
    ...
    place.imageColor = await sampleImageColor(input);
    await writeFile(PLACES_FILE, `${JSON.stringify(data, null, 2)}\n`);
